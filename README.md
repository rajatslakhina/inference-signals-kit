...
Outside the Apple-only `SignpostSink` bridge (`#if canImport(os)`), the core module imports nothing but `Foundation`, and every decision in it is unit-tested on Linux; the bridge itself is compile-checked by the macOS CI job only. The properties this README claims are proven by audits that are also run against deliberately broken implementations.

- **Library:** this repo — `InferenceSignals` (core) + `InferenceSignalsUI` (SwiftUI dashboard)
- **Demo app:** [`inference-signals-kit-demo-app`](https://github.com/rajatslakhina/inference-signals-kit-demo-app) — an `.xcodeproj` consuming this package as a version-pinned remote dependency (`from: "1.1.0"`), driving the dashboard with the simulated planner / executor / reviewer workload. Its README states exactly what was and was not verified; **no Simulator run happened and it contains no screenshots.**

---

## Why this matters

A conventional service has an SLO because its behaviour is a function of its inputs. An inference call is not. It can complete cleanly and be wrong; it can refuse politely and count as success in every HTTP-shaped metric you own; it can call the same tool forty times and return `200`. The Foundation Models profiling session at WWDC26 (session 243) demonstrates the whole argument in miniature — a feature that never errored, never crashed, and quietly did the wrong thing because the wrong instruction was live. Nothing but the trace made that visible.
...
| `SessionTracer` | Mirrors the Instruments tree. Profile and device envelope are frozen at *enqueue*; a mid-request profile switch does not reassign blame. Issues opaque `RequestHandle`s that cannot be replayed against another tracer. Open requests are capped (`maximumOpenRequests`, default 64): enqueueing past the cap cancels the *oldest* with an error record, so an abandoned handle can neither leak nor vanish. `end()` cancels whatever is still open. |
...
| `SignalCollector` | Synchronous, lock-protected ingest (no `await` on the inference path). Aggregates *before* sampling. `flush()` / `flushBatch()` drain under the lock before awaiting the sink, so concurrent flushes deliver every record exactly once and a failed delivery re-offers through the buffer's own bound; `flushBatch()` returns exactly the records this call delivered. Profile map capped with an `_overflow` bucket. |
...
**Consequence, documented:** records with no request (`sessionStarted`, `profileSwitched`, `sessionEnded`) key on the session ID, so a session's lifecycle rows share one deterministic coin and are kept or dropped as a block under a given envelope. A profile switch with no session start to anchor it is not useful to the fleet; the demo's "Last flush" list therefore shows no session rows once the device is hot.
...
### Everything that can grow is capped

The buffer is capped by construction, the profile map folds into `_overflow`, the histogram is `bounds.count + 1` counters, the tool-loop window is `period × repetitions`, and the tracer's open-request map is capped at `maximumOpenRequests` (default 64) — enqueueing past it terminates the *oldest* open request as `.cancelled` with an error record, so an abandoned handle neither leaks its `PromptShape` nor disappears. Lock discipline is uniform: the tracer never holds its own lock while calling the collector, and `SamplingDeciding.decide` is documented as being called under the collector's lock, so it must be pure.

### Attribution is frozen at enqueue

A Dynamic Profile session ([WWDC26 session 242](https://developer.apple.com/videos/play/wwdc2026/242/)) becomes a planner, then an executor, then a reviewer. If the session switches profile while a request is in flight, the request keeps the profile that issued it — the planner that produced a slow request keeps the blame after the session has moved on. The same holds for the device envelope: a request enqueued at `.serious` is a `.serious` request even if the device cools before it completes. `TracerTests.testAttributionIsFrozenAtEnqueue` proves both.
...
swift test                                          # Executed 93 tests, with 0 failures
```

The 93 XCTest cases cover: saturating arithmetic at both ends; identifier validation (spaces, newlines, combining marks, length 65) and re-validation on decode; FNV-1a against published vectors; histogram bucket search, interpolation and the overflow bucket; sampling-policy validation (a rate that *rises* with pressure is rejected); determinism and request coherence (proven at a 50% rate on one kept and one dropped request, not as `false == false` at 2%); the all-or-nothing session-lifecycle key; the three sampling audits against `Sampler` (passes) and against a flat-rate sampler, an incoherent sampler, a half-rate sampler and the unmixed FNV sampler (each fails on the intended violation); buffer class rules, refusal, drain order and a 50 000-offer compaction test whose storage-footprint bound is measured *before* drain (a footprint read after `drain()` is 0 for a leaking buffer too); buffer audits against the shipped buffer, naive drop-oldest and unbounded; tool-loop period 1/2/3 detection, budget exhaustion and the no-false-positive case; tracer timing, frozen attribution, cancel-on-end, the open-request cap (oldest cancelled with a record), invalid transitions, non-transferable handles and a rewound clock; collector aggregation-before-sampling, exactly-once concurrent flush, `flushBatch` returning the batch rather than the cumulative log, failed-delivery re-offer under the buffer bound and the profile cap; wire-format string audit, sorted-key byte-exact encoding and JSON round trip; a reentrant sampler under a watchdog that proves the tracer never calls the collector while holding its own lock; profile switches ignored outside an open session; a degenerate simulated profile with negative ranges that must run without trapping; simulator determinism and reachability of every record class.
...
```swift
.package(url: "https://github.com/rajatslakhina/inference-signals-kit.git", from: "1.1.0")
```
...
Tests/InferenceSignalsTests/  93 XCTest cases, Linux-runnable
```
